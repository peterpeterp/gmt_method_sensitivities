	...
	if len(scenario_files)!=0:
		if var=='tos':
			...
			if len(cdoinfo.split('\n')[1].split(' 0 '))==3:
				Popen('cdo -O selyear,1861/2099 tmp_m_'+var+'.nc tmp_s_'+var+'_tosError.nc',shell=True).wait()
				Popen('cdo -O remapdis,../../blend-runnable/grid1x1.cdo tmp_s_'+var+'_tosError.nc '+var+'_'+scenario+'_tosError.nc',shell=True).wait()
